...
## 2026-09-11
- **✨ Design OS.** Browse three interactive website examples and start a website brief. New setup guide explains Codex and the optional local design library.
- **✨ DeepSeek Harness and Flash Lab.** Run coding tasks in the Harness and explore the game, website, planner and mission-control examples. Updated guide covers connecting your own Token Harbor V4.1 Flash account.
- **🔧 Cleaner member export.** Includes current source and setup tools; removes the owner recording banner, personal sample details and large generated demo videos.

## 2026-09-06
- **✨ Ad Studio.** Write ad variations with Codex, add reference images, and render creatives with your connected Hermes/Grok setup. See `install/44-AD-STUDIO.md`.
- **✨ Ads Performance.** Read Meta campaign results, compare spend and conversions, and download a report. Connect your own account first; see `install/45-ADS-PERFORMANCE.md`.
- **⚡ Updated Mission Control.** The latest dashboard layout and agent status improvements are included, alongside the modern chat interface and GPT-6 Astra support from the previous pack.

## 2026-09-05
- **✨ A more immersive Memory Galaxy.** Explore a glowing spiral of your own notes, with blue and violet stardust, a bright core, and quieter floating controls. Search, filter by folder, see this week's notes, find unlinked memories, or click **Explore** and **Surprise me**. Select a star to see its connections, then **Read memory** to open the note. See `install/11-MEMORY-OBSIDIAN.md`.
- **🔧 Thumbnail generator included in the app.** Thumbnail Studio and thumbnail research now find the bundled generator automatically. Set up your own OpenAI key and Python image library; there is no need to copy scripts into a Claude skill folder. See `install/10-THUMBNAIL-STUDIO.md`.
- **⚡ Current dashboard source and member guides.** This pack contains the current app, Mac launchers, Windows/Linux setup paths, and the updater. Past release details remain in `CHANGELOG.md`.
- **🔧 Clear OpenMontage status.** Fresh installs now get an honest setup message instead of a video job that never completes. Cinematic/Movie pipeline scripts remain a known optional limitation; see `install/26-OPENMONTAGE.md`.
- **🔧 Installation build fix.** Corrected three API exports that could stop the dashboard from building during a fresh install.

## 2026-08-25
- 🔄 **Updates now refresh your guides too.** `Update Agent OS.command` (and both update playbooks) now also bring the new `CHANGELOG.md`, `README.md` and `install/` guides into your existing folder — additively, never touching your own files — so "check the changelog after updating" actually shows the new changelog.
- 🧹 **"Hey Hermes" wake word removed.** The ear toggle in the Hermes chat is gone — hands-free voice is one clear path now: **Hermes-Jarvis** (`3-JARVIS-VOICE.md`). Less to set up, less to break.
- 📎 **Attach files in chat.** The **Claude** and **Hermes** chats now have a paperclip next to the mic — attach docs, images or spreadsheets and the agent reads them with its own file tools before answering. Files are saved to `~/.agentic-os/uploads/` (outside the app, so updates never touch them). No setup. (Thanks Mike for the ask! 🙏)
- 📰 **`UPDATE.md` always tells you the truth now.** Its "new in this version" blurb had gone stale — it now shows the pack's exact version and points you to this changelog (which rolls every build) instead of a hand-written list that could drift.
- 🔧 **OmniRoute key lookup follows YOUR Hermes profile.** The OmniRoute chat's OpenRouter-key fallback now reads your active Hermes profile first instead of assuming a specific profile name existed.

## 2026-08-20
- 🩺 **Hermes → Manage tells you the real problem.** If Hermes isn't installed yet, the embedded Hermes Dashboard panel used to spin on "still starting…" forever. Now it says exactly what's missing and the one line to paste to your AI agent to fix it — and if your Hermes is too old for the `hermes dashboard` command, it tells you to update it. (Thanks to the member who reported this. 🙏)

## 2026-08-17
- 🪟 **Big Windows fix-pack** (thanks to a member's brilliant bug report 🙏):
  - **Agent tabs now detect your CLIs on Windows.** Claude, Codex, Hermes, Kimi, Grok and friends were all showing "not installed" no matter what you had — the auto-detection used a Mac-only command. Fixed, plus Windows install folders are now checked.
  - **NotebookLM Short Video works on Windows.** The generator was crashing with `spawn nlm ENOENT` because of a Mac-style PATH — fixed, along with the same bug in **eleven** other places (every agent now gets a correct PATH, home folder and working directory on Windows).
  - **Notebook chat logs go to YOUR vault.** They were written to a hardcoded `Documents/Obsidian Vault` even if your vault lives elsewhere (any OS) — now they follow your `vaultRoot` config, and skip logging if no vault is set.
  - **Docs fixed: Hermes keys go in `~/.hermes/.env`** (7 places said `~/.hermes/profiles/main/.env`, which Hermes doesn't read — that's what caused `AuthError: No inference provider configured`).
  - **Windows guide: two new warnings** — don't unzip into Google Drive/Dropbox/OneDrive (cloud sync breaks `npm install` with misleading errors), and what to do if `.bat` files open in a text editor → `install/25-WINDOWS-INSTALL.md`.
...
## 2026-08-14
- 🤖 **Hermes gets Bot Mode.** New **Bots** sub-tab in Hermes: build a roster of **named bots** — each with its own chat, avatar, personality, memory and schedule. A bot is a Hermes profile under the hood, so the Hermes desktop app's Bot Mode sees the exact same roster. No setup beyond Hermes itself (`install/4-HERMES.md`).

## 2026-08-13
- 🐭 **New tab: OpenMausBot.** An open-source Mac chat app where you build a **team of named AI bots** — each with its own personality and job — running on the `claude`/`codex`/`grok` CLIs you already have (no new keys). The OS tab mirrors your roster + threads live, and its **shared-memory** toggle gives every bot the same Obsidian brain as Hermes, Claude and the rest of your agents. Bot builds land in a per-bot workspace you can preview. Setup (one .dmg): `install/43-OPENMAUSBOT.md`.

## 2026-08-11
- 🐍 **New tab: Prime Agent.** Prime Intellect's open-source (MIT) coding agent — the model works in a **live Python workspace** so the terminal shows the real code it runs, and **autonomous mode** keeps it working inside budgets (turns/tokens/time) until your pass/fail gates go green. Install + connect your provider: `install/42-PRIME-AGENT.md`.

## 2026-08-10
- 🔥 **Hermes Muse now forges ideas from YOUR channel.** The daily YouTube idea forge (the "furnace" behind Hermes → Muse) reads **your own** channel's winners — set `"youtubeChannel": "https://youtube.com/@YourHandle"` once in `~/.agentic-os/config.json` (or several with `"furnaceChannels"`). If nothing's set, the tab says exactly that instead of failing quietly. Guide: `install/4-HERMES.md`.

## 2026-08-06
- 🌀 **New tab: Muse Code.** Chat + build with **Muse Spark 1.2**, Meta's agent-tuned coding model (1M context), via your existing OpenRouter key — describe a build, get a complete single-file app with live preview. Setup: `install/41-MUSE-CODE.md`.
- 🍁 **Local tab: Maple engine is portable.** The Maple-Preview local engine now finds its model under *your* home folder (`~/maple-mlx`, overridable with `MAPLE_MODEL`/`MAPLE_START`) instead of a hardcoded path.
