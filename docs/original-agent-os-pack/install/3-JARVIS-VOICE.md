...
3. Paste it into your settings. The simplest way: create or open the file `~/.hermes/.env` and add this line (paste your key after the `=`):
   ```
...
- **Set it as the permanent default:** add this line to `~/.hermes/.env` (next to your `ELEVENLABS_API_KEY`), then restart the dashboard:
  ```
