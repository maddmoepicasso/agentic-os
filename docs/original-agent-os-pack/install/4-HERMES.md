...
## What you get
The full **Hermes** tab: Chat, Bots, Hermes-Jarvis (the voice), Hermes Muse, Studio, Sessions, Workspace, MCPs, Manage, Control Room, and Goal Mode. You can say "research X and write me a summary" and it actually does it.
...
Create the file `~/.hermes/.env` and add:
```
...
**💬 Chat** — talk to Hermes like any AI chat. The difference: it can actually DO things (files, web, commands), not just answer. **Attach files with the 📎** next to the mic — docs, images, spreadsheets — and Hermes reads them with its own file tools (the Claude tab has the same 📎).

**🤖 Bots** — Bot Mode: a roster of **named bots**, each with its own chat, personality, memory and schedule. A bot is just a Hermes profile under the hood, so the same roster shows up in the Hermes desktop app's Bot Mode too — create a bot in either place and both see it. No extra setup beyond Hermes itself.

**🔥 Hermes Muse** — your daily YouTube idea forge, powered by "the furnace": it reads **your own channel's** recent videos straight off YouTube (no API key), ranks what's pulling hardest right now, and forges 8 new video ideas that ride your proven patterns. One-time setup: set your channel in `~/.agentic-os/config.json` — `"youtubeChannel": "https://youtube.com/@YourHandle"` (or list several handles with `"furnaceChannels": ["HandleOne", "HandleTwo"]`). No channel set? The tab tells you exactly that. Ideas are forged by Hermes on its free Grok lane.
