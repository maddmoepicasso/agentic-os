...
> ⚠️ **Unzip to a LOCAL folder — never inside Google Drive / Dropbox / OneDrive.** The install writes tens of thousands of files (`node_modules`), and cloud-sync layers choke on that — you'll get weird npm errors that say nothing about the real cause. Desktop or `C:\agent-os` is perfect; if your Desktop/Documents is synced (OneDrive often does this silently), use `C:\agent-os`.
>
> 💡 **A `.bat` file opens in a text editor instead of running?** Some dev tools steal the `.bat` file association. Easy sidestep: in the folder, click Explorer's address bar, type `cmd`, press Enter, then run the file by typing its name.
