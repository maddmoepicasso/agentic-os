...
> ✨ **What's new in this version?** The day-by-day list is **[`CHANGELOG.md`](CHANGELOG.md)** (newest at the top), and the highlights are in the **✨ New in this version** section of `README.md`. Those two are updated every single build — check them after each update to see what you just got.
...
```
```bash
# 4. (recommended) also refresh the guides + changelog in YOUR pack folder — additively, no deletes
#    (run from the new pack's folder; "$APP/.." is your existing pack root)
rsync -a ./README.md ./CHANGELOG.md ./UPDATE.md ./UPDATE-WITH-AI.md ./SETUP-GUIDE.md \
  ./SETUP-WITH-AI.md ./IMPLEMENTATION-CHECKLIST.md ./30-DAY-ROADMAP.md ./DISCLAIMER.md ./VERSION "$APP/.." 2>/dev/null
rsync -a ./install/ "$APP/../install/" 2>/dev/null
```
