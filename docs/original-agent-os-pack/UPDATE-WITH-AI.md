...
**Also refresh the pack's docs** (so the member's `CHANGELOG.md`, `README.md`, `UPDATE.md` and `install/` guides aren't stuck on the old version). Copy the new pack's top-level `*.md` files + `VERSION` + the `install/` (and `extras/`, if present) folders over the member's pack root — **additively** (no delete, and don't touch the `*.command` files or member-created files).
